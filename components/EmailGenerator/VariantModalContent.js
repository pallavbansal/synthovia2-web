...
const VariantModalContent = ({
    ...
    const handleDownload = (variant, index) => {
        ...
        try {
            ...
            const emailTypeRaw = inputs?.email_type || inputs?.emailType;
            const goalRaw = inputs?.goal_or_purpose || inputs?.email_goal || inputs?.emailGoal;

            const emailType =
                typeof emailTypeRaw === 'string'
                    ? emailTypeRaw
                    : emailTypeRaw?.value || emailTypeRaw?.label || 'Email';

            const goal =
                typeof goalRaw === 'string'
                    ? goalRaw
                    : goalRaw?.value || goalRaw?.label || 'Goal';
        ...
        <div style={modalStyles.overlay}>
            <div style={modalStyles.modal}>
                <div style={modalStyles.header}>
                    <h2 style={modalStyles.title}>Generated Email Variants ({variants.length})</h2>
                    <button
                        onClick={onClose}
                        style={{
                            background: 'none',
